...
MAX_DURATION = 160  # Default max attack duration in seconds
...
async def ensure_correct_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if update.effective_chat.id != GROUP_ID:
        await update.message.reply_text(f"❌ 𝐭𝐡𝐢ꜱ 𝐛𝐨𝐭 𝐜𝐚𝐧 𝐨𝐧𝐥𝐲 𝐛𝐞 𝐮ꜱ𝐞𝐝 𝐢𝐧 𝐚 ꜱ𝐩𝐞𝐜𝐢𝐟𝐢𝐜 𝐠𝐫𝐨𝐮𝐩. 𝐣𝐨𝐢𝐧 𝐡𝐞𝐫𝐞:- https://t.me/DDOS_SELLER_12")
        return False
    return True
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    cooldown_time = 1  # Cooldown period in seconds (1 minutes)
    ...
    try:
        ...
        if durations:
            for duration in durations:
                ...
                max_duration = duration.get('max_duration', 160)
...
async def clear_duration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        db["durations"].update_one(
            ...
            {"$set": {"max_duration": 160}},
            upsert=True
        )

        await update.message.reply_text(f"✅ 𝐌𝐚𝐱 𝐝𝐮𝐫𝐚𝐭𝐢𝐨𝐧 𝐫𝐞𝐬𝐞𝐭 𝐭𝐨 16𝟎 𝐬𝐞𝐜𝐨𝐧𝐝𝐬 𝐟𝐨𝐫 𝐮𝐬𝐞𝐫 𝐈𝐃: {target_user_id}.")
    except Exception as e:
